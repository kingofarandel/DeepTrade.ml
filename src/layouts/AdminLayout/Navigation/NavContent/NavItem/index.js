...
const NavItem = ({ layout, item }) => {
    ...
    if (item.external) {
        ...
        subContent = !item.url.includes('discord.gg') ? (
            <NavLink to={item.url} className="nav-link" exact={true} target={itemTarget}>
                <NavIcon items={item} /> {itemTitle} <NavBadge items={item} />
                {item.title === 'Status' && <div style={{ marginLeft: 50, width: 10, height: 10, display: 'inline-block', borderRadius: '50%', backgroundColor: '#34C38F' }} />}
            </NavLink>
        ) : (
            <a href={item.url} className='nav-link'>
                <NavIcon items={item} className="nav-link" /> {itemTitle} <NavBadge items={item} />
            </a>
        );
    }
    let mainContent = '';
